...
if($tag_type!='article' && $tag_type!='video' && $tag_type!='gallery'){
	$tag_type = 'article';
}
?>
<style>
...
<div class="adv_search_result">
	...
	if(isset($_GET['request']) && $_GET['request']=='ALL'):
		...
		$check_archive=@$_COOKIE['archive_'.preg_replace('/[^A-Za-z0-9\-]/', '', substr(md5($term) , 0, 12))];
		if($check_archive==''){
			...
			endforeach;
			setcookie('archive_'.substr(md5($term) , 0, 12),json_encode($hasarchive),time() + (60 * 15));
	...
	if(isset($_GET['request']) && $_GET['request']=='MIN'):
		if($type==''){
			...
			elseif($row_type=='V'):
				$type_query=" (title LIKE '%".$term."%' OR summary_html LIKE '%".$term."%' OR author_name LIKE '%".$term."%')";
			else:
		...
		$check_archive=@$_COOKIE['archive_min_'.$live_db.'_'.substr(md5($term) , 0, 12)];
		if($check_archive==''){
			$hasarchive['archive_result']=[];
			...
			endforeach;
			setcookie('archive_min_'.$live_db.'_'.substr(md5($term) , 0, 12),json_encode($hasarchive),time() + (60 * 15));
			$archivelist=$hasarchive;
		}else{
			$archivelist=json_decode($check_archive,true);
		}
		...
		if(isset($_GET['archive']) && $_GET['archive']==true && $_GET['year']!=''){
			...
			$pattern_arc="SELECT title,url,".$image_path." as article_page_image_path ,summary_html,publish_start_date FROM ".$archive_db_new." WHERE ".$type_query." AND status='P' AND publish_start_date <=NOW() ORDER BY publish_start_date DESC";
	...
	if($tags!=''){
		...
		if($tag_type=='video'){ $image_col='video_image_path as article_page_image_path'; }
		if($tag_type=='gallery'){ $image_col='first_image_path as article_page_image_path'; }
		$cookie_tag=@$_COOKIE['tag_'.$tag_type.str_replace(' ','_',substr(urlencode($tags), 0 ,8))];
		$tag_query="SELECT title,url," .$image_col. " ,summary_html,publish_start_date FROM ".$tag_type." WHERE tags LIKE '%".$tags."%' AND publish_start_date <=NOW() AND status='P' ORDER BY publish_start_date DESC";
		if($cookie_tag==base64_encode($tag_query)){
			...
			setcookie($cookie_tag, base64_encode($tag_query),time() + (60 * 15));
		...
		$row=(isset($_GET['per_page']) && $_GET['per_page'])?$_GET['per_page']:0;
		$pagination=$this->pagination->create_links();
		if(!$this->memcached_library->get($tag_query.'tagresult'.$row) && $this->memcached_library->get($tag_query.'tagresult'.$row) == ''){
			$Result=$this->live_db->query($tag_query." LIMIT ".$row.", 15")->result();
			$this->memcached_library->add($tag_query.'tagresult'.$row,$Result);
		}else{
			$Result = $this->memcached_library->get($tag_query.'tagresult'.$row);
		}
	}
	...
	if($title=='phalke'){
		echo '<img src="https://images.dinamani.com/images/static_img/rajini_tag.png" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if($title=='kamal67'){
		echo '<img src="https://images.dinamani.com/images/static_img/khbannerside.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if($title=='rajinikanth'){
		echo '<img src="https://images.dinamani.com/images/static_img/rajani-banner.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if($title=='budget 2022'){
		echo '<img src="https://images.dinamani.com/special_page/UnionBudget2022.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='local body election'){
		echo '<img src="https://images.dinamani.com/special_page/Local_Body_Election_result1.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='ukraine'){
		echo '<img src="https://images.dinamani.com/special_page/Ukraine.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='5stateresult' or strtolower($title)=='5statesresult'){
		echo '<img src="https://images.dinamani.com/images/static_img/Assembly_Election.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='tn budget'){
		echo '<img src="https://images.dinamani.com/images/static_img/TN-stateBudget.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='sri lanka crisis'){
		echo '<img src="https://images.dinamani.com/images/static_img/Srilanka-inner-changed.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='ramadan'){
		echo '<img src="https://images.dinamani.com/images/static_img/Ramzan-inner.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='president election'){
		echo '<img src="https://images.dinamani.com/images/static_img/PresidentElection-625x50.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='agnipath'){
		echo '<img src="https://images.dinamani.com/images/static_img/Agnipath-innerpage.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='chess olympiad'){
		echo '<img src="https://images.dinamani.com/images/static_img/chess_olympiad_inner.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	if(strtolower($title)=='commonwealth games'){
		echo '<img src="https://images.dinamani.com/images/static_img/commonwealth_inner.jpg" class="img-responsive" style="margin-bottom:20px;">'; 
	}
	print '<table id="example" class="display result-section" cellspacing="0" width="100%">';
	foreach($Result as $fetched):
			$publisheddate    = date('jS F Y', strtotime($fetched->publish_start_date));
