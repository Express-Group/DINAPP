...
class article_details_controller extends CI_Controller {
...
$view_mode            = "live";
	  ...
	  else if(substr($this->uri->uri_string(),-3) == 'ece')
	  {
		...
		if(count($xml)!= 0)
		{
			...
			$header_file        = HOMEURL.'application/views/view_template'.'/'.$common_header_file;
			$header_file_exist  = file_get_contents($header_file);  //get file contents from home.php
			if(($header_file_exist!= '' && $header_file_exist!=false) || $is_common_header==1){  //check file exist
			$data['header'] 	    = $header_file_exist;
                       	}else{
			...
			for($i=1; $i <= $loop_break_point; $i++){
				...
				//echo "<br>-- body Content".$pass_body_content;
			...
			$footer_file        = HOMEURL.'application/views/view_template'.'/'.$common_footer_file;
			$footer_file_exist  = file_get_contents($footer_file);  //get file contents from home.php
			if(($footer_file_exist!= '' && $footer_file_exist!=false) || $is_common_header==1){  //check file exist
			$data['footer'] 	= $footer_file_exist;
			}else{
