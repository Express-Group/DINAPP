...
class rasipalan_controller extends CI_Controller{
		...
		if(isset($_GET['from']) && isset($_GET['type']) && $_GET['type']=='urup' && $_GET['from']=='page'){
			if(count($_POST)==9){
				if(isset($_POST['fname']) && isset($_POST['mname']) &&  isset($_POST['fdob']) &&  isset($_POST['mdob']) &&  isset($_POST['frasipalan']) &&  isset($_POST['mrasipalan']) &&  isset($_POST['fsubrasipalan']) &&  isset($_POST['msubrasipalan']) &&  isset($_POST['email']) && $_POST['fname']!='' &&  $_POST['mname']!='' &&  $_POST['fdob']!=''  &&  $_POST['mdob']!='' &&  $_POST['frasipalan']!='' &&  $_POST['mrasipalan']!='' &&  $_POST['fsubrasipalan']!='' &&  $_POST['msubrasipalan']!='' && $_POST['email']!=''){
					...
					$otp = rand(1000,10000);
					$checkemail = $this->thirumanaporutham_db->query("SELECT tmid , email FROM thirumanaporutham_master WHERE email='".$_POST['email']."' AND manapen_peyar='".$_POST['fname']."' AND manamagam_peyar='".$_POST['mname']."'")->result();
					if(count($checkemail) > 0){
						$date = date('Y-m-d H:i:s');
						$data = ['manapen_peyar'=>$_POST['fname'], 'manamagam_peyar'=>$_POST['mname'],'manapen_dob'=>$_POST['fdob'],'manamagan_dob' => $_POST['mdob'],'manapen_rasi' => $_POST['frasipalan'],'manamagan_rasi' => $_POST['mrasipalan'],'manapen_nachatiram' => $_POST['fsubrasipalan'],'manamagan_nachatiram' => $_POST['msubrasipalan'], 'email' =>$_POST['email'],'otp'=> $otp,'otp_verified'=>0,'email_status'=>0,'result_status'=>0,'created_on'=>$date];
						$this->thirumanaporutham_db->where('tmid',$checkemail[0]->tmid);
						$this->thirumanaporutham_db->update('thirumanaporutham_master',$data);
						$lastInsertedId = $checkemail[0]->tmid;
					}else{
					...
					if($lastInsertedId!=0 && $lastInsertedId!=null){
						 ...
						 $this->input->set_cookie(['name'=>'rsi','value'=>$lastInsertedId]);
	...
	public function sent_otp($email_address, $otp,$rasiid){
		...
		$config['smtp_user'] = 'matching@dinamani.com';
