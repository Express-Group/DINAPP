...
class readwhereApi_controller extends CI_Controller{
	...
	public function article_by(){
		...
		$type = substr($article_id , 0 ,1);
		$contentID = explode('-' , $article_id);
		$year = str_replace(['A','G','V'],'',$contentID[0]);
		$contentOriginalID = $contentID[1];
		if($type=='V'){
			$content_type_id = 4;
			$table = "video_".$year;
		}else if($type=='G'){
		...
		$article_landing_details 	= $this->widget_model->get_content_by_id($contentOriginalID, $content_type_id,"");
	...
	public function mapped_section($contentType , $contentId){
		if($contentType==4){
			$tableName = 'video_section_mapping';
		}else if($contentType==3){
			$tableName = 'gallery_section_mapping';
		}else{
			$tableName = 'article_section_mapping';
		}
		$result = $this->live_db->query("SELECT Sectionname FROM sectionmaster WHERE Section_id IN(SELECT section_id FROM ".$tableName." WHERE content_id='".$contentId."')")->result_array();
		$tmp =[];
		foreach($result as $data):
			array_push($tmp , $data['Sectionname']);
		endforeach;
		return implode(',',$tmp);
	}
