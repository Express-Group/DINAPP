...
class Template_designer extends CI_Controller {
	...
	{			
		...
		else if($this->uri->segment(1) != folder_name)
		{
			...
			else if(((strtolower($this->uri->segment(2)) == "astro_predictions") || (strtolower($this->uri->segment(2))=="numerology"))&& $total==2) 
			{
				if(strtolower($this->uri->segment(2)) == "astro_predictions")
				{
				 $newURL = base_url()."astrology/astro_predictions/aries";
				 redirect($newURL,'location',301);    // redirect astro page to astro/aries page..
				}elseif(strtolower($this->uri->segment(2)) == "numerology"){
				 $newURL = base_url()."astrology/numerology/one";
				 redirect($newURL,'location',301);    // redirect numerology page to numerology/one page..
				}
			...
			else if($content_exist && !empty($last_uri))
			{
				...
				$content_id_from_url     = (count($split_uri)>=2)? explode("-", $split_uri[0]):  explode("-", $split_uri[0]);
				$content_id              = end($content_id_from_url);
				$image_number	         = (count($split_uri)>=2)? $split_uri[1]: 1;
				$content_from     = "live";
			  //  print_r($this->uri->uri_string());exit;
				...
				//print_r(urldecode($current_url));exit;			
		...
		$parent_sectionid = $section_details['ParentSectionID'];
			//print_r($section_details['ParentSectionID']);exit;
