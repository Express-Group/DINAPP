...
{	
	...
	public function index()  {
		...
		$data['template'] 			= 'import_ecenic';
			...
			$picture_xml_file_path = image_url."/ecenic_images/dn-2013-jan-pictures-without-uri.xml";
		...
		$news_xml_file_path = image_url."ecenic_images/article/nie-2013-jan-01-15-news-without-uri.xml";
		...
		$picture_xml_file_path = image_url."ecenic_images/update_ecenic_xml/nie-archive-gallery-nov-24-to-dec-09.xml";
		
		//$xml = get_object_vars(simplexml_load_file($picture_xml_file_path));
		
		$data['GalleryTotalCount']			=  0;//count($xml['Content']);
		
		$data['GalleryTotalSuccess']			= 0;//$this->migration_model->get_ecenic_content('S',3)->num_rows();
		$data['GalleryTotalFailure']			= 0;//$this->migration_model->get_ecenic_content('F',3)->num_rows();
	...
	{	
		...
		if($xml != '' && !empty($xml))
		{
			...
			$Slice_Content  = array_slice($xml['Content'],100,1);
...
$txt = $Contents->ArticleURI."\n";
	...
	{	
		...
		$picture_xml_file_path = image_url."ecenic_images/article/nie-2015-jan-1-15-news.xml";
		...
		if($xml != '' && !empty($xml))
		{
			...
			$Slice_Content  = array_slice($xml['Content'],1,1);
			...
			$ArticleContentDetails['section_promotion'] 			= 0;
	...
	{	
		...
		if($xml != '' && !empty($xml))
		{
			...
			$Slice_Content  = array_slice($xml['Content'],0,50);
			...
			*/
			...
			$Ecenic_Summary = (array)simplexml_load_string(@$SubObject['COM.ESCENIC.SUMMARIES'],'SimpleXMLElement', LIBXML_NOCDATA);
				...
				if($this->migration_model->check_ecenic_content( $Contents->articleid,3) == 0 && (string)$Contents->ArticleState == 'published') {
					...
					if( (string)$Contents->ArticleState != 'published') {
						...
						$this->migration_model->insert_ecenic_content( $Contents->articleid,3 ,'NULL' ,2,(string)$Contents->ArticleURI,(integer)$Contents->sectionID,(string)$Contents->lastModified);
						$Failure++;
					...
					if(isset($Ecenic_Summary['picturerel']) && !(empty($Ecenic_Summary['picturerel']))) {
						...
						if(isset($Result['content_id']) && $Result['content_id'] != 'NULL' && $Result['content_id'] != '') {
							$this->migration_model->insert_ecenic_content( $Contents->articleid,3 ,$Result['content_id'] ,1,$Result['url'],(INT)$Section,(string)$Contents->lastModified); 
						...
						*/
						...
						if(isset($Result['content_id']) && $Result['content_id'] != 'NULL' && $Result['content_id'] != '') {
							...
							$Success++;	
								...
								$this->migration_model->insert_ecenic_content( $Contents->articleid,4 ,'NULL' ,2,(string)$Contents->ArticleURI,(integer)$Contents->sectionID,(string)$Contents->lastModified);
								$Failure++;
						}
