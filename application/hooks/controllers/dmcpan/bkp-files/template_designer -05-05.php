...
class Template_designer extends CI_Controller {
	...
	{	
		...
		//$homepage_details 		= $this->template_design_model->get_template_xmlcontent(1, '');			
		$homepage_details	= $this->template_design_model->get_homePage_xml();
		
		//$template_details 	= $this->template_design_model->load_published_version_template($pageId); 		/// default			
		$template_xml 		= ( count($homepage_details) > 0 && isset($homepage_details['Template_XML']) ) ? @$homepage_details['Template_XML'] : '';
		//PRINT_R($homepage_details ); EXIT;
	...
	public function save_xml_template()
	{
		...
		if($design_lock_status == 2) ////  Template is unlocked  ////
		{
			...
			if($commit_status != 4){
			$emsg 	= $this->template_design_model->update_pagemaster($update_value, $pageId, $publish_xml, $user_id, $use_common_header, $use_common_rightpanel, $use_common_footer, $commit_status, $version_id, $section_id, $page_type);
			...
			else if($commit_status == 4)
			{
				...
				$emsg = $this->template_design_model->make_empty_template_version($update_value, $pageId, $publish_xml, $user_id, $use_common_header, $use_common_rightpanel, $use_common_footer, $commit_status, $version_id, $section_id, $page_type);
			}
			...
			if($publish_xml == "publish")
			{
				...
				if($page_type== 1 && (strtolower($section_details['Sectionname'])=="home")){ // HTML Generation to home page 
		...
		{
			$emsg = array("msg"=>"Current template is locked, actions will not be saved, Please unlock template", "msg_type"=>2, "show_msg"=>1, "trigger_tree"=>"1");				
			header("content-type: application/json"); 
			echo json_encode($emsg);
		}
	...
	public function show_widgetconfig()
	{
		...
		@$data['widget_instance_id']	= trim(array_key_exists("data-widgetid",$request)?$request['data-widgetinstanceid']:'');		
	...
	{		
		...
		/*header("content-type: application/json");
		$templateString_json['header'] = $common_header_checkbox;
		$templateString_json['rightpanel'] = $common_rightpanel_checkbox;
		$templateString_json['footer'] = $common_footer_checkbox;
		echo json_encode($templateString_json); exit;*/
	...
	public function fill_widget($widget_location ,$content)
	{
		...
		$widget_instance_details    = $this->template_design_model->getWidgetInstance('', '','', '', $content['widget_values']['data-widgetinstanceid'], $content['mode'],''); 		
			}
	...
	{
		$page_details 		= $this->template_design_model->getPageDetails($parent_section_id, 'article');		
		...
		{	
			...
			{	
				...
				$page_details 		= $this->template_design_model->getPageDetails($section_details['Section_id'], 'article');								
				$xml				= simplexml_load_string($page_details['templatexml']);
				if(strlen($xml) == 0)
				{					
	...
	public function save_html_template($menuid , $pagetype)
	{
		...
		$is_home_page = (strtolower(@$section_details['Sectionname']) == 'home') ? 'y' : 'n';
		...
		if($xml != '' && !empty($xml))
		{
		   ...
		   $viewpage = $this->load->view("admin/view_frontend", $data, true);
		    if(strtolower(@$section_details['Sectionname'])!="home"){ $ext = ".txt"; }else{ $ext = ".php"; }
	...
	{				
		...
		else if($this->uri->segment(1) == folder_name || $this->input->post("content_ID") != '') ////  From Admin
		{
				//$this->output->cache(1); // for 1 minute ///
				...
				if($this->input->post("content_ID") != '')
				{
					...
					$posted_values		= explode(",",$this->input->post("content_ID"));
					$content_id			= $posted_values[0];
					
					$section_id 		= $posted_values[1];
					$page_type			= $posted_values[2];					
					$content_id				= @$query_string[count($query_string)-1];
					//$url_section_details 	= $this->template_design_model->required_widget_content_by_id($posted_values[0], '1');	///  '1' - for articles	
					$content_det = $this->widget_model->get_content_details($posted_values[0])->result_array();
					$url_section_details 	= $this->template_design_model->required_widget_content_by_id($posted_values[0], $content_det[0]['content_type_id']);						
					$page_details 		= $this->template_design_model->getPageDetails(@$url_section_details[0]['Section_id'], $page_type);		
					$content_id			= $posted_values[0];
		...
		$is_home_page = (@$section_details['Sectionname'] == 'Home') ? 'y' : 'n';
		...
		if($xml != '' && !empty($xml))
		{
			...
			{
			if(@$content_det['content_type_id']=='1' || @$content_det['content_type_id']=='3' || @$content_det['content_type_id']=='4'){
			//print_r($content_det['Content_Id']);exit; 
			$content_id = $content_det['Content_Id'];
			$data['header'] 	= $this->load_saved_template_data($tplheader_values, 'top', "header", $tmpl_values, $content_id, $is_home_page, $static_view, $viewmode, $image_number, $page_type, $page_param,'', $page_details);
			$data['body'] 		= $this->load_saved_template_data($tplheader_values, 'left', "template_body", $tmpl_values, $content_id, $is_home_page, $static_view, $viewmode, $image_number, $page_type, $page_param,'', $page_details);
			$data['footer'] 	= $this->load_saved_template_data($tplheader_values, 'footer', "footer", $tmpl_values, $content_id, $is_home_page, $static_view, $viewmode, $image_number, $page_type, $page_param,'', $page_details);
			}
	...
	public function load_saved_template_data($tplheader_values, $file_type, $file_name, $tmpl_values, $content_id, $is_home_page, $static_view, $viewmode, $image_number,  $page_type, $page_param, $content_from, $page_details)
	{
		$b_section_inc = 0;
		...
		if($page_details['common_header']==1 && $file_type=='top'){
			$commonTemplate_XML	= $this->common_sectionTemplate_XML();
		...
		foreach($tplheader_values as $key => $values)
		{
			...
			{								
				...
				if($file_type == "left") ///// only for Body section content
				{
					...
					$content_det = $this->widget_model->get_content_details($content_id)->result_array();
					$content_type_id = $content_det[0]['content_type_id'];
	...
	public function save_widget_config()
	{
		...
		$widget_customtitle = @$configuration->customTitle;
		...
		$widget_adscript	= @$configuration->iframeUrl;
		//$widget_showsummary	= @$configuration->showSummary;
		$widget_showsummary	= "y";
		$widget_bgcolor		= @$configuration->customBgColor;
		...
		if($widget_instance_id != '' && !empty($widget_instance_id))
		{
			...
			{
				//////  Old code - before introducing temporary instance config  //////
				$update_widget =  $this->template_design_model->update_widgetinstance_renderingmode($widget_instance_id, $rendering_mode, $widget_customtitle, $login_user_id, $widget_section_id, $widget_adscript, $widget_bgcolor, $widget_max_articles, $widget_showsummary, $widget_publishon, $widget_publishoff, $widget_status, $version_id);
			}
			else
			{
		...
		else if($widget_type == 2)
		{
			...
			if(count($simpletab_list) > 0)
			{
				...
				{					
					$simpletab_data['custom_title'] 	= @$value->customTitle;
			...
			if(count($simpletab_list) > 0)
			{
				...
				{					
					...
					foreach($value->childCategory as $subkey => $subCategory)
					{
						...
						$simpletab_sub_data['custom_title'] 	= @$subCategory->customTitle;
	...
	public function show_articles()
	{
		...
		//print_r($section_details); exit;
		$home_value = (strtolower(@$section_details['Sectionname']) == 'home') ? 'home' : 'noHome';
		...
		$edit_flag 	 = '0';
		//if($is_related_article == "no" && ($from_related_article == '' || $from_related_article == 'no')) 
		...
		if($widget_details['widgetStyle'] != 1)
		{
			...
			else
			{
				...
				{
					$this->session->set_flashdata(array("alert_message" => "Configuration is not defined."));										
				}
				else
				{
	...
	public function get_widgetInstancearticle()
	{
		...
		$all_available_articles		= $this->template_design_model->list_out_published_articles($parent_section_id, $data['contenttype_id'], $search_value, $search_by, $from_date, $to_date, $widget_instance);
//echo $this->db->last_query(); exit;
	...
	public function create_widget_instance()
	{
		...
		///////  Save Mainsection config only if widgetstyle is 1, ie., Normal widget only  //////
		/*if($widget_post['widgetStyle'] == 1)
		{
			///////////  Normal  ///////////			
			$normal_tab_form_data 					= "";
			$normal_tab_form_data['Instanceid'] 	= $instanceId;
			$normal_tab_form_data['custom_title'] 	= '';			
			$normal_tab_form_data['section_id'] 	= '';
			$normal_tab_form_data['section_content_type'] = '';
			$normal_tab_form_data['display_order'] 	= "";
			$normal_tab_form_data['created_by'] 	= $user_id;
			$normal_tab_form_data['sub_section'] 	= array();
			$normal_tab_form_data['page_id'] 		= $widget_post['page_id'];
			
			//$normal_tab_form_data['widget_mainsection_id'] 	= $widget_mainsection_id;
			
			$insert_data[] = $normal_tab_form_data;
			$configuration_result 	= $this->template_design_model->insert_widget_configuration($insert_data, '--','--', $widget_post['widgetStyle'], $widget_post['versionId']);
		}*/
		//$instanceId = "1";
	...
	public function widgetarticle_customdetails()
	{
		...
		$custom_details['Summary'] 					= $this->input->post("Summary");
	...
	{				
		...
		else
		{
			...
			/////////  Remove Unchecked article in temporary table  /////////
			//$this->template_design_model->inactivate_temp_articlecustomdetails($custom_details, $user_id);
			/////////////////////////////////////////////////////////////////
			
			/*if($article_postedvalues[0]['individual'] != "individual")
			{
				////////  Arranging records in sequence order  ////////
				$table_name = "widgetinstancecontent_temp";
				$this->template_design_model->display_order_in_sequence($table_name, $article_postedvalues[0]['widget_instance_id'], $article_postedvalues[0]['instance_mainsection_id'],  $article_postedvalues[0]['instance_subsection_id'], $user_id); 							
				///////////////////////////////////////////////////////
			}*/
	...
	public function view_temparticles()
	{
		...
		//$this->add_widget_article('saveTemporary-savepermanent', '');
		
		/*$user_id  = USERID;
		$widget_instance_id 		= $this->input->post("widget_instance_id");
		$main_sction_id 			= $this->input->post("instance_mainsection_id");
		$subSectionConfig_Id 		= $this->input->post("instance_subsection_id");
		//$widget_custom_title		= $this->input->post("widget_instance_title");
		$top_div_class				= $this->input->post("top_div_class");
		$widget_container_class		= $this->input->post("widget_container_class");
		$widget_values				= ((array) json_decode($this->input->post("widget_values")));
		$show_content 				= "";		
		
		
		$widget_instance_details= $this->template_design_model->getWidgetInstance('', '','', '', $widget_instance_id, 'adminview');		
		if($widget_instance_details['WidgetSection_ID'] != '' && $widget_instance_details['WidgetSection_ID'] != "0")
		{
			$widget_section_details = $this->template_design_model->get_section_by_id($widget_instance_details['WidgetSection_ID']);
			$widget_custom_title = ($widget_instance_details['CustomTitle']!= '') ? $widget_instance_details['CustomTitle'] : $widget_section_details['Sectionname'];
		...
		//print_r($widget_section_details); exit;		
		
		
		//print_r($widget_section_details); exit;		
		
		$widget_file_path			= $widget_values['data-widgetfilepath'];
		$this->add_widget_article('saveTemporary', 'view');

		//$this->add_widget_article('saveTemporary-savepermanent', 'view');
		
		
		$widget_content = array("title" => "Name of widget", "widget_values" => $widget_values, "widget_title" => $widget_custom_title ,"mode" => "temporary", "user_id" => $user_id, "widget_container_class" => $widget_container_class );	
		//print_r($widget_content); exit;
		if($this->input->post("show_temp") == "show_temp")
		{
			...
			echo "<style>
						...
						@font-face {
							  ...
							  src: 
							  url('".$css_path."fonts/Lato-Regular/Lato-Regular.eot?#iefix') format('embedded-opentype'),  
							  url('".$css_path."fonts/Lato-Regular/Lato-Regular.woff') format('woff'), 
							  url('".$css_path."fonts/Lato-Regular/Lato-Regular.ttf')  format('truetype'), 
							  url('".$css_path."fonts/Lato-Regular/Lato-Regular.svg#Lato-Regular') format('svg');
							  font-weight: normal;
							  font-style: normal;
						}
						@font-face {
							  ...
							  src: 
							  url('".$css_path."fonts/SourceSansPro-Regular/SourceSansPro-Regular.eot?#iefix') format('embedded-opentype'),  
							  url('".$css_path."fonts/SourceSansPro-Regular/SourceSansPro-Regular.woff') format('woff'), 
							  url('".$css_path."fonts/SourceSansPro-Regular/SourceSansPro-Regular.ttf')  format('truetype'), 
							  url('".$css_path."fonts/SourceSansPro-Regular/SourceSansPro-Regular.svg#Roboto-Regular') format('svg');
							  font-weight: normal;
							  font-style: normal;
						}
				</style>";
			echo '<div class="'.$top_div_class.'" >';					
				echo '<div class="'.$widget_container_class.'" >';				
					$return_widget_content = $this->fill_widget($widget_file_path ,$widget_content);			
					echo $return_widget_content;					
				echo '</div>' ;
			echo '</div><a href="#" class="remodal-close"></a>';
		}*/
	}
	...
	{		
		if(FPM_ADDADVSCRIPTS)
		{
			if(isset($_POST['versionId'])){
				...
				{			
					$script_text			= $this->input->post("add_header_script_save");
					$version_id				= $this->input->post("versionId");
					$page_id				= $this->input->post("page_id");
	...
	public function is_template_unlocked_by_current_user()
	{
		$page_id				= $this->input->post('lockTemplateid');
		$template_lock_details 	= $this->template_design_model->get_locked_template_details('', @$page_id, '');						
	...
	public function lock_widget_articles()
	{
		...
		$user_id			= USERID;
		$return_result	= array("msg"=>"","res_status"=>0,"show_msg"=>"0", "passed_params"=>"Instance Id:".$widgetinstance_id, "msg_type"=>2);
	...
	/////  Lock widget article using widgetinstace id  ///////
	...
	public function check_widget_config_status()
	{
				...
				{		
					...
					else
					{
						...
						if($lock_status != '')  //// this is from ajax call  //////
						{
							...
							{
								$return_result['msg'] 		= "This widget add article status is open";
								$return_result['res_status']= 0;
								$return_result['msg_type'] 	= 2;
								$return_result['show_msg'] 	= 1;
							}
		...
		else
		{
			...
			if(count($template_lock_details) > 0)
			{
				...
				{
					$return_result['msg'] = "Please unlock template";
					$return_result['res_status']= 0;
					$return_result['show_msg'] 	= 1;
		...
		else
		{
			...
			else
			{
				...
				if(count($template_lock_details) > 0)
				{
					...
					if($template_lock_status == 1)
					{
						$widget_lock_status['msg'] 		 = "Please unlock template";
