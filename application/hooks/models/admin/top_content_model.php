	...
	public function top_content_datatable()
	{
		...
		{	
			...
			if($from_date != '' && $to_date != '') 
			{
				...
				if($CheckInYear <= $CurrentYear) {
					...
					{
						$archiev_total_rows =  $archive_db->query('select content_id, title,  status, created_on as Createdon, section_name,  created_by,  parent_section_name from '.$TableName.' WHERE content_id IN("'.$content_id.'") ')->num_rows();
						
						$Total_rows      += $archiev_total_rows;
						$recordsFiltered_archieve = $archive_db->query('select content_id, title,  status, created_on as Createdon, section_name,  created_by,  parent_section_name from '.$TableName.' WHERE content_id IN("'.$content_id.'") ')->num_rows();
						
						$archieve_result = $archive_db->query('select content_id, title,  status, created_on as Createdon, section_name,  created_by,  parent_section_name from '.$TableName.' WHERE content_id IN("'.$content_id.'")  '.$section_str.' '.$status_string.'  ORDER BY '.$order_field.' '.$order.' LIMIT '.$start.','.$length.' ')->result_array();
		...
		if(count($hit_content_id)>0)
		{
			foreach($top_content_manager as $key=> $top_content) 
			{
				...
				$subdata[] = date("d-m-Y h:i:s",strtotime($top_content['Createdon']));
		...
		if($from_date != '' && $to_date != '') 
		{
			if($CheckInYear <= $CurrentYear) 
			{
				...
				foreach($year_range as $val => $year)
				{
					...
					if ($archive_db->table_exists($TableName))
					{
						$archieve_result[$val] = $archive_db->query('select content_id, title,  status, created_on as Createdon, section_name,  created_by as Username,  parent_section_name from '.$TableName.' WHERE content_id IN("'.$content_id.'") '.$status_string.' ')->result_array();
