...
class monthly_predictions extends astro_monthlyPrediction_model
{
	public function get_monthlyPredictions()
	{
		...
		$this->db->reconnect();
		$Total_rows = $this->db->query('CALL get_astro_MonthlyPrediction("","","","")')->num_rows();
		...
		$monthlyPrediction_values =  $this->db->query('CALL get_astro_MonthlyPrediction(" ORDER BY '.$order_field.' '.$order.' LIMIT '.$start.', '.$length.'","'.$from_date.'","'.$to_date.'","'.$rasi_id.'")')->result_array();

		$this->db->reconnect();

		$recordsFiltered =  $this->db->query('CALL get_astro_MonthlyPrediction("","'.$from_date.'","'.$to_date.'","'.$rasi_id.'")')->num_rows();
		$data['draw'] = $draw;
		...
		$Menu_id = get_menu_details_by_menu_name('Monthly Predictions');
		...
		foreach($monthlyPrediction_values as $monthly_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().'dmcpan/astro_monthly_prediction/update_monthly_predictions/'.urlencode(base64_encode($monthly_prediction['prediction_id'])).'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
