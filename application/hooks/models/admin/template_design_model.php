...
class  Template_design_model extends CI_Model {
	...
	{		
		...
		//$result = $this->live_db->query("CALL `update_pagemaster_header_script`('". $id ."','".addslashes($script_text)."') ");
		if(mysql_error() == '')
	...
	{		
		...
		foreach($get_result as $key => $get_multi_section)
		{
			...
			{
				//$this->create_page($get_sec_id, $get_multi_section['Section_landing']);
			...
			foreach($get_multi_section['sub_section'] as $subkey => $subsection_page)
			{
				...
				{
					//$this->create_page($get_subsec_id, $subsection_page['Section_landing']);
				...
				foreach($special_section_details as $splkey => $special_section)
				{
					...
					{
						//$this->create_page($get_splsec_id, $special_section['Section_landing']);
						//echo "<br>".$special_section['Section_landing'];
		...
		/* From CMS db */
		$standard_article 					  = $this->db->query('CALL get_pagemaster_using_sectionid("10000", "")');		
		...
		/* for Clone Template */
		$clone_template 					  = $this->db->query('CALL get_pagemaster_using_sectionid("10001", "")');		
	...
	{		
		...
		if($get_sec_id == 10000)
		{
			$standard_article 			 = $this->live_db->query('CALL get_pagemaster_using_sectionid("10000", "")');		
			...
			$standard_article 			 = $this->live_db->query('CALL get_pagemaster_using_sectionid("10001", "")');		
			$live_table_article_rows	 = $standard_article->num_rows();
			if($live_table_article_rows == 0 )
			{
				$page_insert 	= $this->db->query("SELECT @inserted_id")->row_array();
		...
		/*$standard_article 			 = $this->live_db->query('CALL get_pagemaster_using_sectionid('.$get_sec_id.', "")');		
		$live_table_article_rows	 = $standard_article->num_rows();
		if($live_table_article_rows == 0 )
		{
			$page_insert 	= $this->db->query("SELECT @inserted_id")->row_array();
			...
			foreach($configuration_data as $key => $form_data)
			{
				...
				{					
					foreach($form_data['sub_section'] as $sub_key => $subsection)
					{
						...
						/////// 1-param = subsection_config_id, 2-param = mainsection_config_id
						//$select_query 				= $this->db->query("CALL get_widget_subsection_config('".$sub_config_search_key."', '".$main_config_search_key."')"); 
						//echo $this->db->last_query(); //exit;
						//if($select_query->num_rows() > 0)
						if(false)
						{
							$sub_section_config 		= $select_query->row_array();
							$widget_subsection_id 		= $sub_section_config['WidgetInstanceSubSection_id'];
							
							$query 						= $this->db->query("CALL update_widget_subsection_config('".$custom_title."', ".$section_id.", '".$sub_section_id."', '".$display_order."', '".$created_by."', '".$widget_subsection_id."', '', '".$active_status."')");					
	...
	{		
		...
		if(!$instancecontent_id)
		{
			...
			{
				//$return_msg =  array("status"=>true,"message"=> "ID: ".$content_id." - Article Successfully saved","inserted_id"=>$last_insert_result['@inserted_id'], "instancecontent_id"=>$instancecontent_id); 
		...
		else
		{
			...
			//echo $uploaded_image_name." --- ". $this->db->last_query(); exit;
			if(!mysql_error())
			{
	...
	/*public function addwidget_article($widget_article_details)
	{
		echo "<br>";
	...
	public function articles_searchby_results($searchby,$searchtext,$sectionid,$fromdate,$todate, $tag_id)
	{
		...
		//echo 'CALL articles_searchby_results(" ORDER BY '.$order_field.' '.$order.' LIMIT '.$start_limt.', '.$length.'","'.$check_in.'","'.$check_out.'","'.$Search_value.'","'.$Search_by.'","'.$Section.'","'.$Status.'","'.$content_type.'","'.$tag_id.'","'.$searchby.'")';exit;
	...
	public function required_widget_content_by_id($contennt_ids, $content_type_id)
	{
		...
		//var_dump(" ORDER BY '.$order_field.' '.$order.' LIMIT '.$start_limt.', '.$length.'","'.$check_in.'","'.$check_out.'","'.$Search_value.'","'.$Search_by.'","'.$Section.'","'.$Status.'","'.$content_type.'", "'.$contennt_ids.'");
	...
	{				
		...
		$update_displayorder 				= (@$custom_details['display_order'] == '')? '' : @$custom_details['display_order'];
	 	$update_contet_status 				= (@$custom_details['checked_status'] == '')? '2' : @$custom_details['checked_status']; 
		//echo "<br> status: ".$update_contet_status;
		$user_id 							= $user_id;
		//$insert_date 						= date("Y-m-d H:i:s", time());
		...
		//echo "<br>". $this->db->last_query();
		//echo $temp_articles[0]['instancecontent_tempID']; exit;
		//print_r($temp_articles); 
		...
		{
			//if($temp_articles[0]['Status'] == 1)
			//{
			
				$temp_table_id = $temp_articles[0]['instancecontent_tempID'];
				//$updated_date = date("Y-m-d H:i:s", time());	
				//echo $temp_articles[0]['DisplayOrder']." ** ".$update_displayorder;
				...
				else
				{
					...
					{
						//echo $update_displayorder . " : " . $temp_articles[0]['DisplayOrder']; exit;
						...
						//echo $this->db->last_query();
						//print_r($available_display_order_details); exit;
						$updated_date = date("Y-m-d H:i:s", (strtotime(@$available_display_order_details['publishedon'])-1));
						//echo @$available_display_order_details['publishedon']; exit;
					}
					else
					{
		...
		//echo 	 $this->db->last_query()." - Widget Preview";					exit;	
		//return $this->db->last_query()." - Widget Preview";					
		return $return_msg;					
	}
	...
	public function get_widgetInstanceTempArticles($widget_instance, $mainSectionConfig_Id,  $subSectionConfig_Id, $content_id, $user_id, $related_content_id )
	{
		...
		//$select_query 	= $this->db->query("CALL get_WidgetInstanceTempArticles('','','".$widget_instance."','".$mainSectionConfig_Id."','".$subSectionConfig_Id."')");				
		$result			= $select_query->result_array();		
		//echo $this->db->last_query();
		//print_r($result); exit;
		return $result;
	...
	public function get_content_mapping($mapped_section_id, $content_type_id, $content_id) {
		...
		//$type = $this->db->query("CALL get_content_mapping ('".$content_id."', '".$content_type_id."')");
		$type = $this->db->query("CALL article_content_mapping ('".$content_type_id."','".$mapped_section_id."','".$content_id."')");
		//echo $this->db->last_query();
		//print_r($type->result_array()); exit;
		return $type;
	}
	...
	public function publish_widgetinstance_articles($widget_instance_id, $user_id) 
	{
		...
		//$query = $this->db->query( "CALL publish_widgetinstance_articles ( '".$widget_instance_id."', '".$user_id."' )" );
...
class WidgetConfigLock extends Template_design_model
{
	...
	/////  For getting the widget Config & Article locked status use - 	get_locked_widget_article() function  //////
...
class TemplateVersion extends Template_design_model
{
	...
	public function get_default_versionTemplate($page_id, $return_data)
	{
		...
		//print_r($version_details->row_array()); exit;
	...
	public function insert_widget_configuration_temporarily($configuration_data, $widget_mainsection_id_extras, $widget_subconfig_id_extras, $widget_type, $page_version_id, $page_id )
	{
		...
		if($configuration_data != "" && !empty($configuration_data))
		{
			...
			foreach($configuration_data as $key => $form_data)
			{
				...
				{
					/* if($form_data['section_id'] == ''){
						$update_main_key = $j;
					}
					else{
						$update_main_key			= array_search($form_data['section_id'],$section_id_extra);						
					} */
