...
class Widget_model extends CI_Model
{
  ...
  public function __construct() 
	...
	public function GetMetaData($value){
		...
		public function get_EmbedDetails($contentID){
			$class_object = new Widget_config_class;
		return $class_object->get_EmbedDetails($contentID);
		}
	...
	public function get_highlight_article_by_contentid($content_id, $view_mode='') {
	   $class_object = new Widget_content_class;
		return $class_object->get_highlight_article_by_contentid($content_id, $view_mode='');
	}
 ...
 public function get_content_type($content_id)
	...
	public function GetLiveNewsCount($article_id,$sction_id){
		$class_object = new Widget_config_class;
		return $class_object->GetLiveNewsCount($article_id,$sction_id);
	}
	...
	public function GetFixedArticles($ContentID,$SectionID,$ContentType){
		$class_object = new Widget_config_class;
		return $class_object->GetFixedArticles($ContentID,$SectionID,$ContentType);
	}
	...
	public function Saveneetform(){
		$class_object = new Widget_config_class;
		return $class_object->Saveneetform();
	}
	
	public function get_justinarticles(){
		$class_object = new Widget_config_class;
		return $class_object->get_justinarticles();
	}
...
{
	public function get_justinarticles(){
		return $this->live_db->query("SELECT title,url,publish_start_date,article_page_image_path from article WHERE status='P' AND publish_start_date <= NOW() ORDER BY publish_start_date DESC LIMIT 15")->result();
	}
	public function Saveneetform(){
		$details = json_encode($_POST);
		return $this->db->insert('neet_user_details',['neet_details'=>$details,'status'=>1]);
	...
	public function GetContentBasedOnArchive($limit,$author_name,$totalcount){
		...
		$query .="LIMIT ".$limit.",".$totalcount;
		echo $query;
		return $archive_db->query($query)->result();
	}

	public function get_EmbedDetails($contentID){
		$query=$this->live_db->query("SELECT * FROM article WHERE content_id='".$contentID."' ")->result();
		return $query;
	}
	public function GetFixedArticles($ContentID,$SectionID,$ContentType){
		$Content=array();
		...
		$PrevoiusArticle=$this->live_db->query("SELECT content_id,section_id,title,url,".$ImageName.",summary_html FROM  ".$TableName." WHERE section_id='".$SectionID."' AND content_id <'".$ContentID."' ORDER BY content_id DESC LIMIT 1 ")->result();
		$NextArticle=$this->live_db->query("SELECT content_id,section_id,title,url,".$ImageName.",summary_html FROM  ".$TableName." WHERE section_id='".$SectionID."' AND content_id >'".$ContentID."' ORDER BY content_id  LIMIT 2 ")->result();
		
		if(count($PrevoiusArticle) < 1){
				$NextArticle=$this->live_db->query("SELECT content_id,section_id,title,url,".$ImageName.",summary_html FROM  ".$TableName." WHERE section_id='".$SectionID."' AND content_id >'".$ContentID."' ORDER BY content_id  LIMIT 3 ")->result();
		}
		if(count($NextArticle) ==1){
				$PrevoiusArticle=$this->live_db->query("SELECT content_id,section_id,title,url,".$ImageName.",summary_html FROM  ".$TableName." WHERE section_id='".$SectionID."' AND content_id <'".$ContentID."' ORDER BY content_id DESC LIMIT 2 ")->result();
		}
		if(count($NextArticle) ==0){
				$PrevoiusArticle=$this->live_db->query("SELECT content_id,section_id,title,url,".$ImageName.",summary_html FROM  ".$TableName." WHERE section_id='".$SectionID."' AND content_id <'".$ContentID."' ORDER BY content_id DESC LIMIT 3 ")->result();
		}
 ...
 public function get_liveContents_by_sectionId($max_article, $section_id, $view_mode, $lead_story_maximum_Articles , $subsec_leadstory_remdering_mode, $leadstory_contentID, $contentType_id,$is_home)
{
 ...
 public function get_RightSide_OtherStories_Contents_Type3($max_article, $section_id, $view_mode,$sectionid,$parentsectionid)
 {
	...
	$length =100;
	if($content_type==7){
		$rss_article =$this->db->query("SELECT Question_id,Questiontext,AnswerInPlainText FROM askprabhu WHERE Status='1' LIMIT 100")->result_array();
	}else{
...
class Widget_content_class extends Widget_config_class {
	...
	public function get_highlight_article_by_contentid($content_id, $view_mode='') {
		...
		$hightlights =  $this->live_db->query("SELECT highlights_content from article_highlights WHERE content_id= '".$content_id."'");
		return ['count' =>$hightlights->num_rows() , 'result' => $hightlights->result() ];
	}
