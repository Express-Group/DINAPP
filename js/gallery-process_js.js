...
$('#count2').keyup(function() {
	...
	if(folder_name == 'niecpan') {
		...
		CKEDITOR.replace( 'txtgalleryname',
			...
			$.ajax({
			url: base_url+folder_name+"/gallery_manager/get_publish_history_popup", // Url to which the request is send
			type: "POST",             // Type of request to be send, called as method
			data:  postdata,
			dataType: "HTML",
			async: false, 
	...
	var inst = $.remodal.lookup[$('[data-remodal-id=version_model]').data('remodal')];
				...
				var postdata = "first_contentversion_id="+version_array[0]+"&second_contentversion_id="+version_array[1];
			$.ajax({
			url: base_url+folder_name+"/gallery_manager/view_version_compare", // Url to which the request is send
			type: "POST",             // Type of request to be send, called as method
			data:  postdata,
			dataType: "HTML",
			async: false, 
			...
			CKEDITOR.replace( 'first_version_galleryname',{
      	 	toolbar : [ { name: 'basicstyles', items: [ 'Bold', 'Italic', 'TextColor' ] } ]
    		});
	
	
			CKEDITOR.replace( 'second_version_galleryname',{
       		toolbar : [ { name: 'basicstyles', items: [ 'Bold', 'Italic', 'TextColor' ] } ]
    		});
			...
			CKEDITOR.replace( 'second_version_summary', {
					...
					Flash_message("Please select only 2 versions for comparision","SessionError");
		}
					...
					if(data.status == 1) {
						...
						if(folder_name != 'niecpan') 
						 ...
						 if(parseInt(Index+1)%2 != 0)
							...
							$("#ImageContainerSet").append('<div class="ImageClose" id="SetImage'+data.image_id+'"><img src="'+data.image+'" /><img id="ImageRemove" data-image_id="'+data.image_id+'" src="'+base_url+'images/admin/close-button.png" /></div>'); 
